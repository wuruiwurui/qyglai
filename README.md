...
- 按模块控制接口与菜单访问权限。
- 角色支持 `all`、`dept_tree`、`dept`、`self` 数据范围，后端根据当前登录用户、所属组织和角色自动拼接查询条件。
- 文件、合同、发票、对账、销售跟进、客服工单、报表、知识库空间、复核任务和流程实例列表均已接入数据权限过滤。
- 新上传文件、AI 抽取生成的合同/发票、对账批次和报表会自动写入负责人或所属组织，保证后续查询、审计和交付现场能按部门或个人隔离数据。
- 文件详情、字段确认、字段修正历史、人工复核完成、流程实例详情和流程催办等按 ID 操作的接口已增加后端可见性校验，避免绕过列表直接访问越权数据。
...
- 工作流待办中心支持按节点、实例、处理人搜索，并可筛选全部待办、已逾期和今日到期任务；逾期任务会以醒目标识提示。
- 流程实例列表支持按审批中、已通过、已驳回、已撤回状态过滤，并可按流程名称、业务类型、实例ID和当前节点搜索。
- 流程设计器新增校验问题面板，发布前会展示节点编码重复、缺少名称、处理时限异常、审批人ID格式错误和条件配置不完整等问题。
- 审批操作进一步防呆：驳回必须填写审批意见，转交必须填写目标用户ID，后端同时校验不能转交给自己或非法用户ID。
